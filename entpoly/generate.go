...
func (e *Extension) generate(g *gen.Graph) error {
	...
	// Optional sidecar .graphql emission when WithGQLSchemaFile is set
	// AND at least one MorphTo.GQL() is in scope. The .graphql file is
	// fully generated — atomically overwritten each pass.
	if e.gqlSchemaPath != "" && data.HasGQL {
		if werr := writeGQLSchema(e.gqlSchemaPath, data.GQLChildren); werr != nil {
			return fmt.Errorf("entpoly: write gql schema: %w", werr)
		}
	}
	return nil
}

// writeGQLSchema emits a `.graphql` file containing one `union <Name>
// = <P1> | <P2>` declaration per GQL-enabled child relation.
func writeGQLSchema(path string, children []childData) error {
	var buf bytes.Buffer
	buf.WriteString("# Code generated by entpoly. DO NOT EDIT.\n")
	buf.WriteString("#\n")
	buf.WriteString("# GraphQL union declarations for every MorphTo(...).GQL()\n")
	buf.WriteString("# relation in the project. Pair with the Go-side type aliases +\n")
	buf.WriteString("# Is<Union>() markers in polymorphic.go and a gqlgen resolver\n")
	buf.WriteString("# wired through the GQL<Rel>(ctx) helper.\n\n")
	for _, c := range children {
		buf.WriteString("union ")
		buf.WriteString(c.GQLUnionName)
		buf.WriteString(" = ")
		for i, p := range c.AllowedTypes {
			if i > 0 {
				buf.WriteString(" | ")
			}
			buf.WriteString(p)
...
func (e *Extension) buildTmplData() (*tmplData, error) {
	...
	for _, c := range sortedChildren {
		...
		gqlName := c.GQLUnionName
		if c.GQL && gqlName == "" {
			gqlName = pascalCase(c.MorphName)
		}
...
type tmplData struct {
	...
	// HasGQL is true when at least one child declared GQL().
	HasGQL bool

	// GQLChildren is the subset of Children that opted into GraphQL
	// union emission.
	GQLChildren []childData
...
type childData struct {
	...
	// parent.
	GQL            bool             // True when MorphTo(...).GQL() was set — emits the
	// gqlgen-recognisable union surface (type alias
	// + exported Is<Union> markers + resolver helper).
	GQLUnionName   string           // PascalCase union name (e.g. "Commentable").
