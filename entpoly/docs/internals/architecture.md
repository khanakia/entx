...
| 18 | Type column is `field.String` (no `MixinAllowed`) | `preprocess.handleMorphTo` detects whether the type column carries `Enums` and threads `TypeIsEnum` through `childInfo` / `parentInfo` / `holderInfo`. The template branches at every cast site so plain-string columns emit `string(MorphKey)` directly while enum columns keep the named-type conversion. | `TestRender_TypeIsEnumFalse_OmitsPredicateCast` + `TestRender_TypeIsEnumTrue_KeepsCast` + the `TestRender_Matrix_*` cases |
| 19 | `MorphedByMany.Through(...)` morph-name resolution from the pivot | A pre-pass over `g.Nodes` (before any edges are stripped) builds `polyState.pivotMorph[typeName] = morphName`. `handleHolder` then resolves: explicit `.MorphName(...)` > pivot's MorphTo > `singularise(ThroughName)` fallback. Closes the gap where `Through("source_links", SourceLink.Type)` would default to `"source_link"` even when the pivot's `MorphTo` was `"sourceable"`. | `TestPreprocess_MorphedByMany_ResolvesMorphNameFromPivot` + `_ExplicitMorphNameWins` + `_FallbackToSingularise` |
| 20 | Composite-index storage-key override (cross-module collision) | `MixinIndexName(name)` option sets `index.Fields(typeCol, idCol).StorageKey(name)`. Default ent index naming continues for everyone not hitting the collision. | `TestMixinIndexName_StorageKeyApplied` + `_DefaultUnset` |
...
| Soft-delete-aware reverse resolve | ✅ `.SoftDelete()` opt-in; auto-detects per-parent which targets declare the field, filters them via `<field>IsNil()` in resolver + eager-load |
