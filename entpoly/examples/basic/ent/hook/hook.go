...
// The TaggableFunc type is an adapter to allow the use of ordinary
// function as Taggable mutator.
type TaggableFunc func(context.Context, *ent.TaggableMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TaggableFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TaggableMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TaggableMutation", m)
}
