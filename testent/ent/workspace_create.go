...
func (_c *WorkspaceCreate) AddFolderIDs(ids ...int) *WorkspaceCreate {
	_c.mutation.AddFolderIDs(ids...)
	return _c
}

// AddFolders adds the "folders" edges to the Folder entity.
func (_c *WorkspaceCreate) AddFolders(v ...*Folder) *WorkspaceCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
func (_c *WorkspaceCreate) AddDocIDs(ids ...int) *WorkspaceCreate {
	_c.mutation.AddDocIDs(ids...)
	return _c
}

// AddDocs adds the "docs" edges to the Doc entity.
func (_c *WorkspaceCreate) AddDocs(v ...*Doc) *WorkspaceCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddDocIDs(ids...)
}
...
func (_c *WorkspaceCreate) createSpec() (*Workspace, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.DocsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
