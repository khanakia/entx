...
func (_q *TagQuery) QueryPostTags() *PostTagQuery {
...
func (_q *TagQuery) WithPostTags(opts ...func(*PostTagQuery)) *TagQuery {
	query := (&PostTagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withPostTags; query != nil {
		if err := _q.loadPostTags(ctx, query, nodes,
			func(n *Tag) { n.Edges.PostTags = []*PostTag{} },
			func(n *Tag, e *PostTag) { n.Edges.PostTags = append(n.Edges.PostTags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadPostTags(ctx context.Context, query *PostTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *PostTag)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(posttag.FieldTagID)
	}
	query.Where(predicate.PostTag(func(s *sql.Selector) {
