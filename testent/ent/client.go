...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		c.Article, c.AuditLog, c.Category, c.Channel, c.Comment, c.Doc, c.Folder,
		c.Member, c.Note, c.Post, c.PostTag, c.Profile, c.Revision, c.Tag, c.Team,
		c.User, c.Workspace,
	} {
		n.Use(hooks...)
	}
...
// QueryTeam queries the team edge of a AuditLog.
func (c *AuditLogClient) QueryTeam(_m *AuditLog) *TeamQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, auditlog.TeamTable, auditlog.TeamColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
func (c *TeamClient) QueryAuditLogs(_m *Team) *AuditLogQuery {
...
type (
	hooks struct {
		Article, AuditLog, Category, Channel, Comment, Doc, Folder, Member, Note, Post,
		PostTag, Profile, Revision, Tag, Team, User, Workspace []ent.Hook
	}
	inters struct {
		Article, AuditLog, Category, Channel, Comment, Doc, Folder, Member, Note, Post,
		PostTag, Profile, Revision, Tag, Team, User, Workspace []ent.Interceptor
	}
)
