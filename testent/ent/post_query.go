...
func (_q *PostQuery) QueryPostTags() *PostTagQuery {
...
func (_q *PostQuery) WithPostTags(opts ...func(*PostTagQuery)) *PostQuery {
	query := (&PostTagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PostQuery) loadPostTags(ctx context.Context, query *PostTagQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostTag)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(posttag.FieldPostID)
	}
