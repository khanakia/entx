...
func (_q *NoteQuery) QueryDoc() *DocQuery {
...
func (_q *NoteQuery) WithDoc(opts ...func(*DocQuery)) *NoteQuery {
	query := (&DocClient{config: _q.config}).Query()
...
func (_q *NoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Note, error) {
	...
	if query := _q.withDoc; query != nil {
		if err := _q.loadDoc(ctx, query, nodes, nil,
			func(n *Note, e *Doc) { n.Edges.Doc = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NoteQuery) loadDoc(ctx context.Context, query *DocQuery, nodes []*Note, init func(*Note), assign func(*Note, *Doc)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "doc_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *NoteQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withDoc != nil {
			_spec.Node.AddColumnOnce(note.FieldDocID)
		}
	}
