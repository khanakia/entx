...
func (_q *ArticleQuery) QueryRevisions() *RevisionQuery {
...
func (_q *ArticleQuery) WithRevisions(opts ...func(*RevisionQuery)) *ArticleQuery {
	query := (&RevisionClient{config: _q.config}).Query()
...
func (_q *ArticleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Article, error) {
	...
	if query := _q.withRevisions; query != nil {
		if err := _q.loadRevisions(ctx, query, nodes,
			func(n *Article) { n.Edges.Revisions = []*Revision{} },
			func(n *Article, e *Revision) { n.Edges.Revisions = append(n.Edges.Revisions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ArticleQuery) loadRevisions(ctx context.Context, query *RevisionQuery, nodes []*Article, init func(*Article), assign func(*Article, *Revision)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(revision.FieldArticleID)
	}
	query.Where(predicate.Revision(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(article.RevisionsColumn), fks...))
	}))
