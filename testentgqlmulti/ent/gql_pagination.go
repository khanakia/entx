...
// ChatbotEdge is the edge representation of Chatbot.
type ChatbotEdge struct {
	Node   *Chatbot `json:"node"`
	Cursor Cursor   `json:"cursor"`
}

// ChatbotConnection is the connection containing edges to Chatbot.
type ChatbotConnection struct {
	Edges      []*ChatbotEdge `json:"edges"`
	PageInfo   PageInfo       `json:"pageInfo"`
	TotalCount int            `json:"totalCount"`
}

func (c *ChatbotConnection) build(nodes []*Chatbot, pager *chatbotPager, after *Cursor, first *int, before *Cursor, last *int) {
	...
	var nodeAt func(int) *Chatbot
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *Chatbot {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *Chatbot {
	...
	c.Edges = make([]*ChatbotEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &ChatbotEdge{
			Node:   node,
...
// ChatbotPaginateOption enables pagination customization.
type ChatbotPaginateOption func(*chatbotPager) error

// WithChatbotOrder configures pagination ordering.
func WithChatbotOrder(order *ChatbotOrder) ChatbotPaginateOption {
	if order == nil {
		order = DefaultChatbotOrder
	}
	o := *order
	return func(pager *chatbotPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultChatbotOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithChatbotFilter configures pagination filter.
func WithChatbotFilter(filter func(*ChatbotQuery) (*ChatbotQuery, error)) ChatbotPaginateOption {
	return func(pager *chatbotPager) error {
		if filter == nil {
			return errors.New("ChatbotQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type chatbotPager struct {
	reverse bool
	order   *ChatbotOrder
	filter  func(*ChatbotQuery) (*ChatbotQuery, error)
}

func newChatbotPager(opts []ChatbotPaginateOption, reverse bool) (*chatbotPager, error) {
	pager := &chatbotPager{reverse: reverse}
	...
	if pager.order == nil {
		pager.order = DefaultChatbotOrder
	}
	return pager, nil
}

func (p *chatbotPager) applyFilter(query *ChatbotQuery) (*ChatbotQuery, error) {
	if p.filter != nil {
...
func (p *chatbotPager) toCursor(_m *Chatbot) Cursor {
	return p.order.Field.toCursor(_m)
}

func (p *chatbotPager) applyCursors(query *ChatbotQuery, after, before *Cursor) (*ChatbotQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultChatbotOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *chatbotPager) applyOrder(query *ChatbotQuery) *ChatbotQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultChatbotOrder.Field {
		query = query.Order(DefaultChatbotOrder.Field.toTerm(direction.OrderTermOption()))
	}
...
func (p *chatbotPager) orderExpr(query *ChatbotQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		...
		if p.order.Field != DefaultChatbotOrder.Field {
			b.Comma().Ident(DefaultChatbotOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to Chatbot.
func (_m *ChatbotQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...ChatbotPaginateOption,
) (*ChatbotConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newChatbotPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	...
	conn := &ChatbotConnection{Edges: []*ChatbotEdge{}}
...
var (
	// ChatbotOrderFieldName orders Chatbot by name.
	ChatbotOrderFieldName = &ChatbotOrderField{
		Value: func(_m *Chatbot) (ent.Value, error) {
			return _m.Name, nil
		},
		column: chatbot.FieldName,
		toTerm: chatbot.ByName,
		toCursor: func(_m *Chatbot) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.Name,
			}
...
func (f ChatbotOrderField) String() string {
	var str string
	switch f.column {
	case ChatbotOrderFieldName.column:
		str = "NAME"
	}
	return str
}
...
func (f ChatbotOrderField) MarshalGQL(w io.Writer) {
	io.WriteString(w, strconv.Quote(f.String()))
}

// UnmarshalGQL implements graphql.Unmarshaler interface.
func (f *ChatbotOrderField) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("ChatbotOrderField %T must be a string", v)
	}
	switch str {
	case "NAME":
		*f = *ChatbotOrderFieldName
	default:
		return fmt.Errorf("%s is not a valid ChatbotOrderField", str)
	}
	return nil
}

// ChatbotOrderField defines the ordering field of Chatbot.
type ChatbotOrderField struct {
	// Value extracts the ordering value from the given Chatbot.
	Value    func(*Chatbot) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) chatbot.OrderOption
	toCursor func(*Chatbot) Cursor
}

// ChatbotOrder defines the ordering of Chatbot.
type ChatbotOrder struct {
	Direction OrderDirection     `json:"direction"`
	Field     *ChatbotOrderField `json:"field"`
}

// DefaultChatbotOrder is the default ordering of Chatbot.
var DefaultChatbotOrder = &ChatbotOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &ChatbotOrderField{
		Value: func(_m *Chatbot) (ent.Value, error) {
			return _m.ID, nil
		},
		column: chatbot.FieldID,
		toTerm: chatbot.ByID,
		toCursor: func(_m *Chatbot) Cursor {
...
// ToEdge converts Chatbot into ChatbotEdge.
func (_m *Chatbot) ToEdge(order *ChatbotOrder) *ChatbotEdge {
	if order == nil {
		order = DefaultChatbotOrder
	}
	return &ChatbotEdge{
