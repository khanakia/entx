...
var chatbotImplementors = []string{"Chatbot", "Node"}
...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	switch table {
	case chatbot.Table:
		query := c.Chatbot.Query().
			Where(chatbot.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, chatbotImplementors...); err != nil {
				return nil, err
			}
		}
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case chatbot.Table:
		query := c.Chatbot.Query().
			Where(chatbot.IDIn(ids...))
		query, err := query.CollectFields(ctx, chatbotImplementors...)
		if err != nil {
			return nil, err
		}
