# enttui — a schema-driven terminal UI (TUI) and admin browser for ent / entgo (Go)

**Generate a full terminal UI / admin dashboard for any [ent](https://entgo.io) (entgo) project, straight from your Go schema — zero UI code.**

`enttui` is a code generator + runtime that turns an existing **ent ORM** schema into a fast, keyboard-driven **terminal UI** for browsing, searching, filtering, sorting, and editing your database. Think **`k9s` for your ent models**: point it at the same `./schema` directory you already pass to `entc.Generate`, run one extra codegen pass, and every entity becomes a navigable, type-safe view — no hand-written admin panels, no reflection, no web server.

Works with any ent SQL dialect (**SQLite, PostgreSQL, MySQL**), any ID type (`string`, `int`, `uuid.UUID` — per entity), and any terminal ([tview](https://github.com/rivo/tview)/tcell: iTerm2, kitty, alacritty, Terminal.app, tmux, ssh).

> Looking for: an **ent admin UI**, an **entgo TUI / database browser**, a **Go terminal data explorer**, a **k9s-style ent dashboard**, or a **no-code CRUD UI for ent**? That's what this is.
...
`enttui` is part of the **entx** toolkit for ent ([github.com/khanakia/entx](https://github.com/khanakia/entx)). It runs as a standalone codegen pass — it does **not** modify or replace your existing `entc` generation.
...
- [Step 2 — register the generated code with your ent client](#step-2--register-the-generated-code-with-your-ent-client)
...
**How do I add an admin UI to an ent (entgo) project?**
Run the `enttui` codegen pass over your existing `./schema`, call the generated `RegisterAll(app, client)`, and run the app. You get a browse/search/filter/sort/edit TUI for every entity without writing any UI or web admin code.

**Is there a `k9s` for databases / for ent?**
Yes — `enttui` is effectively a k9s-style terminal explorer for your ent models: a list+preview browser, fuzzy kind picker, vim navigation, a `,` leader/which-key menu, and a capabilities matrix, all driven by your schema.

**Can it edit data (create / update / delete), or is it read-only?**
Both. Read-only by default; opt into mutations per entity with `enttui.AllowCreate{}`, `enttui.AllowDelete{}`, and `enttui.Editable{}` on fields. Includes a reference picker for foreign keys and bulk copy/export.

**Does it support string, int, and UUID primary keys?**
Yes — ID handling is per entity. A table with an `int` PK, one with a `string` PK, and one with `uuid.UUID` can coexist; the generated code converts each correctly.

**Which databases work?**
Anything ent supports — **SQLite, PostgreSQL, MySQL** (and other ent SQL dialects). `enttui` only talks to your `*ent.Client`, so the database is whatever you already configured.

**Does it need a web server or browser?**
No. It's a pure terminal UI (tview/tcell). No HTTP server, no JS, no DB migrations, runs over SSH/tmux.

**Is it type-safe / does it use reflection?**
The generated accessors are typed Go closures produced from your schema at codegen time — no runtime reflection over your models.

**Does it change my existing ent codegen?**
No. `enttui.Generate(...)` is a separate pass shaped like `entc.Generate(...)`; your `entc` output is untouched.

**Dark / light theme?**
Yes — built-in Catppuccin Mocha (dark, default) and Latte (light), toggled live; semantic palette keeps contrast in both.

**Can I use it from a non-Go app?**
No — it generates Go and links your Go `*ent.Client`. It's for Go/ent projects.

**Try it in 60 seconds:** clone [entx](https://github.com/khanakia/entx) and run `task -d testenttui run` — a seeded, in-memory SQLite demo with string/int/uuid entities. See [`testenttui/`](../testenttui).
