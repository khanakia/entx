...
├── register_memory.go
└── …                      # ≈ one per ent.Type with a project_id field
```
...
When an input field has focus (filter, picker), single-letter shortcuts go to the input — `k` types a `k`, not "open picker." `esc` always closes the input.
...
- Only schemas with a `project_id` field are browsable.
